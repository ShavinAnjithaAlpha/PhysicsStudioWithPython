...
window = Tk()
window.title("electric field line creator")
window.geometry("1200x750")
window.config(bg="black")
window.iconbitmap("pathcreator_logo.ico")

cw = 880
ch = 560

canvas_frame = Canvas(window , width=1000, height=700, bd=0, highlightthickness=0, bg="black" ,scrollregion=(0, 0, 1200, 1000))
canvas_frame.grid(row=0, column=0)
# set the vertical scrol bar to canvas_frame
scroll_ver = Scrollbar(window, orient=VERTICAL, command=canvas_frame.yview)
scroll_ver.grid(row=0, column=1, sticky=NS)
# set the horizontal scrol bar to canvas_frame
scroll_hor = Scrollbar(window, orient=HORIZONTAL, command=canvas_frame.xview)
scroll_hor.grid(row=1, column=0, sticky=EW)
# binding the scroll bar to canvas
canvas_frame.config(yscrollcommand=scroll_ver.set, xscrollcommand=scroll_hor.set)
# create the main frame 
root = Frame(window, bg="black")
canvas_frame.create_window(0, 0 ,window=root, anchor="nw")
...
def create_rectangle(x1, y1, x2, y2, **kwargs):
    if 'alpha' in kwargs:
        ...
        canvas.create_image(x1, y1, image=images[-1], anchor='nw', tag=kwargs.pop('tag'))
        print(kwargs.pop('tag'))
...
def display_x_and_y(event):
    ...
    try:
        # function to check the position on the canvas in the clicked_marker
        get_object = canvas.find_closest(event.x, event.y)[0]
        get_tag = canvas.gettags(get_object)[0]
        if get_tag == "clicked_marker":
            canvas.delete("transparent_box")
            create_rectangle(event.x+5, event.y+5, event.x+200, event.y+150, fill="green2", alpha=0.2, tag="transparent_box")
        else:
            canvas.delete("transparent_box")
    except:
        None
...
def update_canvas():
    ...
    for atom in atoms:
        ...
        canvas.create_oval((-ox - R+ atom[0]) * rx, ch - (-oy + R+atom[1]) * ry, (-ox + R+ atom[0]) * rx,ch - (-oy - R+atom[1]) * ry, outline="green2", fill="green2",width="1.5", tag=f"part{atom[3]}")
        if atom[2]>0:
            canvas.create_text((-ox  + atom[0]) * rx, ch - (-oy  + atom[1]) * ry,text="+", fill="red", tag=f"part{atom[3]}", font=('vedana', 14))
        else:
            canvas.create_text((-ox + atom[0]) * rx, ch - (-oy + atom[1]) * ry, text="-", fill="red", tag=f"part{atom[3]}", font=('verdana', 14))
...
def update_skipregion():
    ...
    for item in atoms:
        ...
        skip_regions.append([item[3]])

def insert_charged_particle():
    global num_of_atom
    if entry_charge.get():
        atoms.append([float(scalex.get()), float(scaley.get()), float(entry_charge.get()), num_of_atom])
        num_of_atom += 1
...
def check_in_the_particle(x,y):
    ...
    for p in atoms:
        ...
        if distance_p < R:
            ...
            angle, i = return_angle(sin, cos), p[3]
...
def start_the_draw(l):
    ...
    while startvar.get() == "start":
        ...
        if a != 0:
            ...
            # suggest path color by electric field intensity
            pathcolor_new = suggest_pathcolor(a)

            canvas.create_line((alpha_1["x"]-ox)*rx, ch-(alpha_1["y"]-oy)*ry, (last_x-ox)*rx, ch-(last_y-oy)*ry, fill=pathcolor_new , width="0.5", tag="path")
            alpha_1["x"] = last_x
            alpha_1["y"] = last_y
        else:
            # create the neutral point as the circle on the canvas
            canvas.create_oval((alpha["x"]-ox-1)*rx, ch-(alpha["y"]-oy+1)*ry, (alpha["X"]-ox+1)*rx, ch-(alpha["y"]-oy-1), fill="yellow", outline="orange", tag="path")
            # set the next x, y to suitable values
            alpha_1["x"] += 0.5
            alpha_1["y"] += 0.5
...
def set_new_atomslist(id, A):
    global num_of_atom
    new_A = []
    i = 0
    for item in A:
        if item[3] != id:
            new_A.append([item[0], item[1], item[2], i])
            i += 1
    # set the count variable to correct position
    num_of_atom = len(new_A)
    return new_A
...
def remove_item():
    ...
    global clicked_id
    if clicked_id >= 0:
       # update the atoms list by remove the clicked id atom
       atoms = set_new_atomslist(clicked_id, atoms)
...
def select_particle(event):
    ...
    if a != 0:
        ...
        canvas.delete("clicked_marker")
        canvas_info.create_line(canvas_info_w/2, canvas_info_h/2, end_x, end_y, tag="electric_arrow", fill="blue", arrow="last", width="2")
        # draw the clicked position on the canvas
        canvas.create_oval(event.x-3, event.y-3, event.x+3, event.y+3, fill='white', outline="white", tag="clicked_marker")
        canvas.create_oval(event.x-5, event.y-5, event.x+5, event.y+5, fill=None, outline="blue", tag="clicked_marker")
        ...
        if angle <= 180:
            ...
            canvas_info.create_text(canvas_info_w / 2, canvas_info_h / 2 + 50, text=f"({round(get_originalxy(event.x, event.y)[0],3)} , {round( get_originalxy(event.x, event.y)[1],3)})",tag="a", fill="white", font=('verdana', 10))
        else:
            canvas_info.delete("a")
            canvas_info.create_text(canvas_info_w / 2, canvas_info_h / 2 - 20, text=f"intensity = {round(a, 3)}", tag="a" , fill="white", font=('verdana', 10))
            canvas_info.create_text(canvas_info_w / 2, canvas_info_h / 2 - 35, text=f"angle = {round(angle,3)}",
...
# Button(frame1, text="start", bg="green2", font=('Helvetica', 10), width="10", relief=FLAT, command=start_the_draw).grid(row=0, column=0, pady=1)
...
Button(frame1, text="draw fields lines", bg="red", font=('Helvetica', 10), wraplength="100", relief=FLAT, command=automation).grid(row=2, column=0, pady=1)
...
frame2.grid(row=6, column=0, pady=5)
...
scaley = Scale(root, from_=-27, to=27, orient=HORIZONTAL, length="300", command=set_position_y, relief=FLAT, sliderrelief=FLAT, bg="black", fg="gray", bd=0, highlightthickness=0, width="10", activebackground="red")
scaley.grid(row=3, column=1)
Label(root, text="Y-position", bg="black", fg="white").grid(row=3, column=0)

scalex = Scale(root, from_=-45, to=45, orient=HORIZONTAL, length="300", relief=FLAT, sliderrelief=FLAT, bg="black", fg="gray", command=set_position_x, bd=0, highlightthickness=0, width="10", activebackground="red")
...
frame_display.grid(row=3, column=3, rowspan=3, padx=2, pady=5)
...
canvas_info_w = 150
canvas_info_h = 150
canvas_info = Canvas(frame_info, width=canvas_info_w, height=canvas_info_h, bg="#323744", relief=FLAT, bd=0, highlightthickness=0)
canvas_info.grid(row=0, column=0, pady=5)
